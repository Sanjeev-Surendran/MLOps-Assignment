...
def load_data_into_db():
    ...
    try:
        ...
        if not check_if_table_has_value(cnx, 'loaded_data'):
            print("Loading dataframe from " + CSV_FILE)
            df_lead_scoring = pd.read_csv(CSV_FILE, index_col=[0])

            print("Replacing null values in columns total_leads_droppped and referred_lead with zero...")    
            ...
            print("Storing processed dataframe to loaded_data table")    
            df_lead_scoring.to_sql(name='loaded_data', con=cnx, if_exists='replace', index=False)    
        else:
            print('loaded_data has been already created')
    except Exception as e:
        print (f'Exception thrown in funtion load_data_into_db : {e}')
    finally:
        if cnx:        
            cnx.close()
...
def map_city_tier():
    ...
    try:
        ...
        if not check_if_table_has_value(cnx, 'city_tier_mapped'):
            ...
            # Dropping city_mapped as it is not required anymore
            df_lead_scoring = df_lead_scoring.drop(['city_mapped'], axis = 1)

            print("Storing modified dataframe to table city_tier_mapped")
            df_lead_scoring.to_sql(name='city_tier_mapped', con=cnx, if_exists='replace', index=False)
        else:
            print('city_tier_mapped has been already created')
    except Exception as e:
        print (f'Exception thrown in funtion map_city_tier : {e}')
    finally:
        if cnx:        
            cnx.close()
...
def map_categorical_vars():
    ...
    try:
        ...
        if not check_if_table_has_value(cnx, 'categorical_variables_mapped'):
            ...
            print('Mapping insignificant variables present in first_platform_c, first_utm_medium_c, first_utm_source_c')
            ...
            print('categorical_variables_mapped has been already created')
    except Exception as e:
        print (f'Exception thrown in function map_categorical_vars : {e}')
    finally:
        if cnx:        
            cnx.close()
...
def interactions_mapping():
    ...
    try:
        ...
        if not check_if_table_has_value(cnx, 'model_input') or not check_if_table_has_value(cnx, 'interactions_mapped'):
            ...
            # Remove 'app_complete_flag', if it is not present in dataframe
            # This flag will help to avoid issues during training and inference
            ...
            print("Selecting a smaller subset of columns for model training part, excluding created_date")
            
            # These columns were derived after rapid expermentation where we excluded columns with relatively low significance
            # Excluding columns with relatively significance indicated in schema.py
            df_cleaned = df_pivot[id_vars[1:]]
            print("Storing schema required df to table model_input")
            df_cleaned.to_sql(name='model_input', con=cnx, if_exists='replace', index=False) 
        else:
            print('model_input and interactions_mapped have already been created')
    except Exception as e:
        print (f'Exception thrown in function interactions_mapping : {e}')
    finally:
        if cnx:        
            cnx.close()
